...
// `retired` stops the sweep re-fetching a URL that will never serve its document again. The entry and
// its captures stay, because a dead URL is evidence of where a party published and that it is gone.
export const trackedSchema = z.array(z.strictObject({url,state,party:slug.nullable(),kind:z.enum(kinds),label:z.string().min(1),publisher_type:z.enum(['party','parliamentary_group','election_authority','parliament','government','press']),added_at:timestamp,found_on:url,retired:z.strictObject({at:timestamp,reason:z.string().min(1)}).optional()}));

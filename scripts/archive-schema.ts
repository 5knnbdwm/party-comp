...
export const captureSchema = z.strictObject({id:z.string(),url,final_url:url.nullable(),retrieved_at:timestamp,http_status:z.number().int().min(100).max(599).nullable(),content_type:z.string().nullable(),bytes:z.number().int().nonnegative().nullable(),sha256:hash.nullable(),text_sha256:hash.nullable(),blob:z.string().nullable(),text:z.string().nullable(),http_last_modified:z.string().nullable(),http_etag:z.string().nullable(),pdf_info:z.strictObject({title:z.string().nullable(),creation_date:z.string().nullable(),mod_date:z.string().nullable(),pages:z.number().int().positive()}).nullable(),wayback_url:url.nullable(),previous_capture:z.string().nullable(),content_changed:z.boolean().nullable(),text_changed:z.boolean().nullable(),error:z.string().nullable()});
